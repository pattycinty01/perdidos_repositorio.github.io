...
# **Factores que influyen en la baja empleabilidad femenina a nivel mundial en el 2019**
...
El modelo cumple con el supuesto de linealidad, pues el promedio del residuo es menor a 0
...
Interpretación del modelo elegido: Modelo 2

El modelo elegido es el número 2 y para probar nuestra hipótesis debemos apreciar que el efecto que tiene la "Tasa de empleo vulnerable" es significativo con respecto a 0.1, ya que la magnitud con la que se indica a este es de -0.063 el cual indica cuánto varía el desempleo de las mujeres en promedio cuando la tasa de empleo vulnerable aumenta en una unidad, esto siendo controlado por el PBI. Ahora si bien se observa un efecto significativo este es inverso, ya que el coeficiente calculado resulta ser negativo.

Por otro lado, el R Cuadrado ajustado (0.1299) nos brinda un porcentaje que es multiplicado por 100 para obtener una pista de la cercanía que se puede tener de una situación cerca (cuando vale 1). Asimismo, luego de haber hecho los diagnósticos correspondientes de la regresión de dicho modelo, en primer lugar, hallamos que la linealidad se encuentra presente en el modelo lo que terminaría significando que servirá para explicar las mismas variables con datos diferentes de otros estudios. En segundo lugar,, en el diagnóstico de homocedasticidad se asume que la dispersión de los errores en la estimación de la tasa de desempleo de la mujer mantiene una variación homogénea,esto al ser comprobado con el test de Breusch-Pagan en donde el estadístico sale 5.8242 con un p-valor de 0.05436, de ahí que se ve que no muestra un sesgo en los resultados y un valor mayor a 0.05 se demuestra su homocedasticidad.

En tercer lugar, en el diagnóstico de la normalidad de residuos se obtiene que los residuos se distribuyen de manera normal con respecto a la distancia entre el valor esperado y el observado de las variables, ya que esto se llega a confirmar con el uso de la prueba de Shapiro-Wilk con un valor de 0.88818 y un p-valor 8.191e-09, los cuales demuestran ser mayores a 0.05 como lo estima el test. En cuarto lugar, con respecto a la no multicolinealidad, sabemos que se da en este caso al existir predictores con una correlación muy alta entre sí. Este modelo es menor a 5, por lo tanto, puede ser problemático, pues evita calcular de forma correcta el efecto de cada regresor.
...
Se observa poca esperanza de una analisis factorial.

### Verificación de si los datos permiten factorizar:
...
##### Nota: Probablemente el modelo está teniendo problemas para factorizas algunas de las variables. Sin embargo, nos muestar que se puede factorizar en un factor.
...
##### Nota: Se muestra, como también lo constata el gráfico, que algunas variables aportan a la factorización. No obstante, prevalece las que no aportan.

```{r}
sort(resfa$complexity)
```

##### Nota: Con esta prueba se verifica que las variables no contribuyen a más de un factor.
